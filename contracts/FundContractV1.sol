    ...
    function depositFund(uint256 fundType_) external payable {
        ...
        for (uint256 index = 0; index < AllFundDetails.length; index++) {
            if (AllFundDetails[index].fundType == fundType_) {
                AllFundDetails[index].totalAmountDeposited =
                    AllFundDetails[index].totalAmountDeposited +
                    msg.value;
        ...
        emit deposit(
            msg.sender,
            msg.value,
            isDeposited ? "ETH deposited" : "FundTypeNotFound"
        );
    }

    function evaluavateNFTCredentials(uint256[] memory credentials_)
        external
        view
        returns (uint256[] memory fundtypes_)
    {
        ...
        for (uint256 index = 0; index < credentialLength; index++) {
            ...
            require(
                ...
                "Credentails not owned by msg.sender"
            );
        }
        //return matched funddetails
        uint256[] memory matchedFundTypes = new uint256[](
            AllFundDetails.length
        );
        for (uint256 index = 0; index < credentialLength; index++) {
            bytes4 identifier = nftFactoryV1.getIdentifiers(
                credentials_[index]
            );

            for (
                uint256 indexOFfundType = 0;
                indexOFfundType < AllFundDetails.length;
                indexOFfundType++
            ) {
                if (
                    identifier ==
                    AllFundDetails[indexOFfundType].credentailsUsed
                ) {
                    matchedFundTypes[index] = AllFundDetails[indexOFfundType]
                        .fundType;
                }
            }
        }
        ...
        for (
            ...
            if (fundType == AllFundDetails[indexOFfundType].fundType) {
                if (
                    ...
                    require(
                        amountDisbursedFromFundByaTeam[
                            AllFundDetails[indexOFfundType].fundType
                        ][msg.sender] == 0,
                        "Already Withdrawed for this fund"
                    );
                    require(
                        AllFundDetails[indexOFfundType].totalAmountDeposited -
                            AllFundDetails[indexOFfundType]
                                .totalAmountDisbursed >
                            AllFundDetails[indexOFfundType]
                                .maxDistributableAmountPerTeam,
                    ...
                    amountDisbursedFromFundByaTeam[
                        AllFundDetails[indexOFfundType].fundType
                    ][msg.sender] = AllFundDetails[indexOFfundType]
                        .maxDistributableAmountPerTeam;
