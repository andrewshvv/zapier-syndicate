...
contract NftFactoryV1 is ERC721URIStorage, Ownable, Pausable {
    ...
    */
    ...
    function mintAnItem(address to_, string memory tokenURI_,bytes4 credentialIdentifier) external returns(uint256){
        require(minters.contains(msg.sender),"Sorry you dont have minter role");
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256 _tokenId) internal view override {
        if (paused()){
            require(_msgSender() == owner() || minters.contains(_msgSender()), "Only the Owner can Transfer or Mint until pause");
        }
    }
    ...
    function getCredentials(address owner_) public view returns(uint256[] memory){
        require(ownerCredentials[owner_].length > 0, "Sorry no Credentials on your wallet");
